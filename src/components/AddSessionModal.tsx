...
function AddSessionModal({ isOpen, onClose = () => {}, sessionId }: AddSessionModalProps) {
  ...
  return createPortal(
    <div className="h-full w-full absolute bottom-0 top-0 right-0 left-0 flex justify-center items-center z-10 bg-[#0000007d]">
      <div role="dialog" aria-labelledby="title" className="bg-light-gray w-full max-w-2xl px-4 mx-4 rounded-sm relative lg:px-6 lg:m-auto py-10">
        <Button aria-label="close" onClick={onClose} className="absolute right-4 top-6">
          <Cross />
        </Button>
